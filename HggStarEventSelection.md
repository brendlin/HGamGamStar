...
| **Overlap removal**    | Remove e if &Delta;R(e,&gamma;)<0.4<br> Remove jet if &Delta;R(j,&gamma;)<0.4<br>Remove jet if &Delta;R(j,e)<0.2<br>Remove e if &Delta;R(j,e)<0.4<br>Remove &mu; if &Delta;R(&mu;,&gamma;)<0.4<br>Remove jet if &Delta;R(&mu;,j)<0.4 | HGamAnalysisFramework/<br>OverlapRemovalHandler.cxx | N/A |
...
|17, LEP_MEDID             | Resolved e: Medium,<br>Merged e: Merged **TMVA** V3 ID,<br>&mu;: Medium | HGamAnalysisFramework/HGamRel21.config<br>for merged objects | Muons: add ID x TTVA SFs<br>Resolved e: add ID x Reco SFs |
...
|19, LEP_ISO               | Resolved e: CloseByCorrected FCLoose, lead ele ONLY<br>Merged e: FCTight<br>&mu;: CloseByCorrected FCTightTrackOnly_FixedRad<br>on lead muon ONLY | Merged e: specially done in<br>HiggsGamGamStarCutflowAndMxAOD.cxx.<br>Resolved e/&mu;: HGamAnalysisFramework/HGamRel21.config | Muons and resolved-e: add Iso SFs |
...
|22, ZMASSCUT              | m<sub>ll</sub> < 30 GeV<br> (see code for merged mass definiton) | HiggsGamGamStarCutflowAndMxAOD.cxx | " " |
